...
**보고자 결정**: 입력 보고자가 비었으면 `currentUser()`. 지정됐으면 이름/이메일을 Jira MCP(`lookupJiraAccountId`/유저 검색)로 **accountId 해석** 후 `reporter in (...)`.
JQL:
```
# 기본(본인)
project = {프로젝트키} AND issuetype = Bug AND reporter = currentUser() ORDER BY created ASC
# 보고자 지정 (1명 이상)
project = {프로젝트키} AND issuetype = Bug AND reporter in ("{accountId1}","{accountId2}") ORDER BY created ASC
```
