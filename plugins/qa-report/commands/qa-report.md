---
description: N차 통합테스트 결과 보고서(.xlsx) 자동 생성 — 입력 한 항목씩 인터랙티브 수집 + Jira fetch + 결정론적 렌더
---
...
## 1. 입력 수집 (한 항목씩 인터랙티브 — 한 번에 다 묻지 말 것)
아래 항목을 **하나씩 순서대로** 질문한다. 한 항목을 질문하면 **사용자 답을 받을 때까지 멈추고**(턴 종료), 답을 받은 뒤 **다음 항목 하나만** 질문한다. 절대 여러 항목을 한꺼번에 나열해 묻지 않는다.
- 사용자가 한 번에 여러 개를 묶어 답하면 그건 그대로 매핑하고, **빠진 것만** 이어서 한 건씩 묻는다.
- 필수 5개(1~5) 답을 모두 받기 전에는 fetch로 진행하지 않는다.
...
5. **출력 폴더 경로**
6. (선택) **보고자(reporter)** — 비우면 실행 본인(currentUser), 특정 인물(들) 지정 시 그 사람 등록 버그로 집계 (이름/이메일/계정, 복수 가능)
7. (선택) **Build Ver.**
8. (선택) **결함 원인 유형 커스텀필드명** (기본값 `결함 원인 유형`)
9. (선택) **이슈 유형(issuetype)** — 기본값 `Bug`. Jira가 다른 이름을 쓰면 지정 (영문 `Bug`가 아니라 한글 유형명일 수 있음 — 예: `오류/버그`·`버그`·`결함`)
...
```
project = {프로젝트키} AND issuetype = "{이슈유형}" AND reporter = currentUser()
  AND created >= "{start}" AND created <= "{end} 23:59" ORDER BY created ASC
```
- `이슈유형`은 입력값(기본 `Bug`). 이름에 `/`·공백이 있으면 **따옴표 필수**(`"오류/버그"`).
- 수집: `key`, `summary`, `status`(statusCategory), **결함 원인 유형 커스텀필드 값**. 필드 못 찾으면 `cause_field_available=false`.
- ⚠ **0건이면 이슈 유형 이름 불일치를 먼저 의심**한다: issuetype을 뺀 `project={프로젝트키} AND reporter=... ORDER BY created DESC`로 최근 몇 건의 **실제 issuetype 이름**을 확인하고, 다르면 사용자에게 그 이름으로 (1)을 재조회한다(영문 `Bug`가 아닌 한글 유형명 가능). 진짜 0건일 때만 "이슈 없음" 처리.
...
```json
{
  ...
  "meta": {
    ...
    "generated_marker": "qa-report v0.1.4"
  }
}
```
