...
9. (선택) **이슈 유형(issuetype)** — 기본값 `오류/버그`. Jira가 다른 이름을 쓰면 지정 (예: `Bug`·`버그`·`결함`)
...
```
- `이슈유형`은 입력값(기본 `오류/버그`). 이름에 `/`·공백이 있으면 **따옴표 필수**(`"오류/버그"`).
...
```json
{
  ...
  "meta": {
    ...
    "generated_marker": "qa-report v0.1.5"
  }
}
```
