...
> Scouter 1.0/2.0은 외부 사용자에게 설명하기 위한 제품형 릴리즈 라벨입니다. 내부 구현 버전은 qa-scout v0.2.x 이력을 따릅니다.

### 기능정의서 품질율

품질율은 기능 커버리지 중심의 내부 지표입니다. 산식은 "재검증 후 기대 기능 대비 기능정의서에 반영된 기능 비율"을 기준으로 하고, `[자료 부족]` 마커 정합성은 보조 지표로 함께 봅니다.

| 릴리즈 라벨 | 내부 기준 | 기능정의서 품질율 | 근거 |
|---|---|---:|---|
| Scouter 1.0 | qa-scout v0.2.5 실전 적용 기준선 | 약 62% | 대표 프로젝트 재검증에서 기능 누락 약 38% 확인. 40 FR 산출물에서 운영 가이드 기반 기능 약 25건(시뮬레이션·결재함·CoverPage 관리·메뉴 관리 등)이 누락되고, `[자료 부족]` 마커 9/10건이 부적절했습니다. |
| Scouter 2.0 | qa-scout v0.2.6~v0.2.7 개선 기준 | 약 95%+ | 단계 12a 커버리지 자가 검증, 자료 부족 self-check, operations-guide 카테고리, 다중 매핑, archive 정책으로 재산출 범위가 40 FR에서 약 65 FR로 확대되고 자료 부족 마커가 9건에서 1~2건 수준으로 축소됐습니다. |
...
Scouter 3.0 품질율은 다음 개선 작업 완료 후 동일 기준으로 실측해 추가합니다.
