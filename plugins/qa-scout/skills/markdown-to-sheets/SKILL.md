...
description: QA가 단계 17a에서 호출하는 스킬. 개발자가 인계한 feature-spec/ markdown 5개를 QA 본인 Google 계정의 Google Sheets 5시트로 자동 이행한다. google-sheets MCP 활용 — create_spreadsheet (옵션 A 신규) 또는 batch_update_cells (옵션 B 사전 공유 시트). feature-spec.yaml 메타 자동 생성 + share_spreadsheet로 개발자 editor 권한 부여 (단계 18c 검수 준비).
---
...
- **04_변경이력 갱신**: 이행 시점 + QA ID 행 추가 (markdown V0.1 → Sheets V0.1+이행)
